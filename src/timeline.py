...
def demo(text):
   ...
   listing=tag(text)
   #print listing[]
   #p = re.compile('\<TIMEX2>(.*?)\</TIMEX2>')
   #text='<TIMEX2>' + timex + '</TIMEX2>', text
   #print len(p.findall(listing))!=0
   #for i in range(len())
   #if (len(p.findall(listing))!=(0))==True:
   #    new.append(p.findall(listing)[0])
   #print new
   return listing
...
def timeline(n):
   ...
   #print wikipedia.search(input)[0]
   page = wikipedia.page("google")
   blob = TextBlob(page.content)
   #print len(blob.sentences)
   for i in range(len(blob.sentences)):
       list.append(demo(str(blob.sentences[i])))
   #p = re.compile('\<TIMEX2>(.*?)\</TIMEX2>')
   #rint list
   p = re.compile('\<TIMEX2>(.*?)\</TIMEX2>')
   #print p.findall(list)
   for i in range(len(list)):
       #p = re.compile('\<TIMEX2>(.*?)\</TIMEX2>')
       #p = re.compile('\<TIMEX2>(.*?)\</TIMEX2>')
       #rint p.findall(listing)
       if (len(p.findall(list[i]))!=(0))==True:
           new.append(p.findall(list[i])[0])
           mylist = set(new)
         #  print type(mylist)
   #print len(list)
   print " ** Top 100 sentences based on chronological order **"
   for i in range(100):
       #p = re.compile('\<TIMEX2>(.*?)\</TIMEX2>')
       #p = re.compile('\<TIMEX2>(.*?)\</TIMEX2>')
       #rint p.findall(listing)
      # print p.findall(list[i])[0] in mylist
       if (len(p.findall(list[i]))!=(0))==True:
           #print p.findall(list[i])[0] in mylist
           if (p.findall(list[i])[0] in mylist) == True:
               #print "harish"
               texts.append(list[i])
   #if p.findall(list[i])[0] in mylist ==True:
   #             p.findall(list[i])
